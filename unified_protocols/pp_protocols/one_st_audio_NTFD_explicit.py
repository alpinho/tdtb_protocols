# one_st_audio_NTFD_explicit.py — Audio No-Temporal (explicit instruction)
...
from stimuli_setup import (
    ...
    get_soft_beep, soft_beep_available, soft_beep_error, BEEP_SOFT_FILE,
    startkey, option1, option2,
    stim_duration, intertrial, onsettime, baselinetime
)

from utilities import notemporal_rating_py, play_isi_explicit, is_beat_trial, load_training_inputs, load_inputs
...
def load_notemp_strict(subject_id, session_id, run_number, session_type='training'):
...
if expInfo is None:
    ...
    dlg = gui.DlgFromDict(expInfo, title='Audio Notemporal (explicit)')
    if not dlg.OK:
        psychopy.core.quit()
...
if xf.tell() == 0:
    ...
    xf.write(f"#e Task: st audio NTFD (explicit)\n")
...
def _play_soft_beep():
    ...
    t_soft = ms_now()
    st.pulse_audio()
    _play_with_hw_time(get_soft_beep())
...
try:
    ...
    for i, trial in enumerate(conditions, start=1):
        ...
        # with softer beeps. isi_5 = T is a single unit; the high/low TARGET below
        # is a separate stimulus and is never subdivided.
        ...
        for j, isi_ms in enumerate(isi_ms_list, 1):
            ...
            play_isi_explicit(
                j, int(round(isi_ms)), t_beep_ms, audio_duration_ms, base_unit, is_beat,
                _play_soft_beep,
                lambda *row: write_row([session_id, block_no, tnum, trial_label, *row]),
                wait_ms, ms_now)
