...
from utilities import notemporal_rating_py, is_beat_trial, SUBINTERVAL_TAG, load_training_inputs, load_inputs
...
from stimuli_setup import (
    ...
    hide_cursor,
    rect, small_rect, SMALL_RECT_CONDITION, triangle, circle
)
...
def load_notemp_visual_strict(subject_id, session_id, run_number, session_type='training'):
...
def show_small_rect():
    ...
    return t_on_ms, float(t_off_ms - t_on_ms), t_off_ms
...
def _cli_exp_info():
    ...
    try:
        ...
        raise ValueError(
            "Usage: python one_st_visual_NTFD_explicit.py [pc_type] "
...
if expInfo is None:
    ...
    dlg = gui.DlgFromDict(expInfo, title='Visual NTFD (explicit)')
    if not dlg.OK:
        psychopy.core.quit()
...
for stim in (rect, small_rect, triangle, circle, purple_cross, black_cross, gray_cross):
    try:
        stim.autoDraw = False
    except Exception:
        pass
...
if xf.tell() == 0:
    ...
    xf.write(f"#e Task: st visual NTFD (explicit)\n")
...
try:
    ...
    for i, trial in enumerate(conditions, start=1):
        ...
        base_unit = (int(round(isi_ms_list[0])) if isi_ms_list else 0)
        ...
        for j, isi_ms in enumerate(isi_ms_list, 1):
            ...
            split_interval = (
                is_beat and base_unit and
                th_ms % int(base_unit) == 0 and
                th_ms // int(base_unit) >= 2
            )

            if split_interval:
                n_sub = th_ms // int(base_unit)
                ref_onset = float(t_on_ms)
                ref_dur = float(STIM_MS)
                sub_onset = float(t_gap_ms)

                for s in range(1, n_sub):
                    wait_ms(max(0.0, float(base_unit) - float(ref_dur)))
                    t_small, small_dur, small_off = show_small_rect()

                    write_row([session_id, block_no, tnum, trial_label,
                               f"interval_{j}{SUBINTERVAL_TAG}{s}",
                               sub_onset, float(t_small - sub_onset),
                               float(base_unit), float(t_small - ref_onset), '-', '-'])
                    write_row([session_id, block_no, tnum, trial_label,
                               SMALL_RECT_CONDITION, t_small, small_dur, '-', '-', '-', '-'])

                    ref_onset = float(t_small)
                    ref_dur = float(small_dur)
                    sub_onset = float(small_off)

                wait_ms(max(0.0, float(base_unit) - float(ref_dur)))
                t_sub_end = ms_now()
                write_row([session_id, block_no, tnum, trial_label,
                           f"interval_{j}{SUBINTERVAL_TAG}{n_sub}",
                           sub_onset, float(t_sub_end - sub_onset),
                           float(base_unit), float(t_sub_end - ref_onset), '-', '-'])
            else:
