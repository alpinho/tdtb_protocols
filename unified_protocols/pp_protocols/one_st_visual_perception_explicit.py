...
from utilities import perception_rating_py, is_beat_trial, SUBINTERVAL_TAG, load_training_inputs, load_inputs
...
from stimuli_setup import (
    ...
    hide_cursor,
    rect, small_rect, SMALL_RECT_CONDITION
)
...
def load_perception_visual(subject_id, session_id, run_number, session_type='training'):
...
def _cli_exp_info():
    ...
    try:
        ...
        raise ValueError(
            "Usage: python one_st_visual_perception_explicit.py [pc_type] "
...
if expInfo is None:
    ...
    dlg = gui.DlgFromDict(expInfo, title='Visual Perception (explicit) - strict TSV')
    if not dlg.OK:
        psychopy.core.quit()
hide_cursor()
...
if xf.tell() == 0:
    ...
    xf.write(f"#e Task: st visual PERCEPTION (explicit)\n")
    ...
    for i, tr in enumerate(conditions, start=1):
        ...
        for k, ioims in enumerate(isis, start=1):
            split_interval = (
                ...
                int(ioims) % int(base_unit) == 0 and
                int(ioims) // int(base_unit) >= 2
            )

            if split_interval:
                n_sub = int(ioims) // int(base_unit)
                ref_onset = float(prev_on_ms)
                ref_dur = float(last_flash_dur)
                sub_onset = float(last_off_ms)

                for s in range(1, n_sub):
                    wait_ms(max(0.0, float(base_unit) - float(ref_dur)))
                    t_small, small_dur, small_off = show_small_rect()

                    w(xw, [session_id, block_no, tnum, trial_label,
                           f'interval_{k}{SUBINTERVAL_TAG}{s}',
                ...
                wait_ms(max(0.0, float(base_unit) - float(ref_dur)))
                final_sub_onset = sub_onset
                final_ref_onset = ref_onset
            else:
            ...
            if split_interval:
                w(xw, [session_id, block_no, tnum, trial_label,
                       f'interval_{k}{SUBINTERVAL_TAG}{n_sub}',
                       final_sub_onset, float(t_next_on - final_sub_onset),
                       float(base_unit), float(t_next_on - final_ref_onset), '-', '-'])
            else:
