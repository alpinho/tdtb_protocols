# one_st_audio_perception.py — Audio Perception
...
def load_perception(subject_id, session_id, run_number, session_type='behavioral'):
...
if expInfo is None:
    ...
    dlg = gui.DlgFromDict(expInfo, title='Audio Perception')
    if not dlg.OK:
        psychopy.core.quit()
...
if xf.tell() == 0:
    ...
    xf.write("#e Task: st audio PERCEPTION\n")
...
try:
    ...
    for i, tr in enumerate(conditions, start=1):
        ...
        # For each isi_k: wait (isi_k - prev_beep_dur_measured), then next beep.
        for k, th_ioi_ms in enumerate(isis, start=1):
            # isi onset is previous beep OFFSET (measured)
            isi_on = prev_off_ms

            # wait gap = IOI - measured beep duration (Expyriment semantics)
            gap_wait = max(0.0, float(th_ioi_ms) - float(prev_dur_ms))
            wait_ms(gap_wait)

            # next beep onset (measured)
            t_on2 = ms_now()
            st.pulse_audio()
            _play_now(audiowav_medium)

            realized_o2o = t_on2 - prev_on_ms           # onset→onset realized
            isi_dur_meas = t_on2 - isi_on               # offset→next onset measured gap
            ...
            write_row(xw, [session_id, block_no, tnum, trial_label, f"interval_{k}",
                        isi_on, isi_dur_meas, th_ioi_ms, realized_o2o, "-", "-"])
