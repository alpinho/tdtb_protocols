# one_st_audio_production_explicit.py — Audio production (explicit instruction)
...
from utilities import production_rating_py, play_isi_explicit, is_beat_trial, load_training_inputs, load_inputs
...
if not soft_beep_available:
    ...
    print("\n*** " + reason + " ***\n")
    try:
        psychopy.core.quit  # noqa  (ensure psychopy.core is imported)
...
def _cli_exp_info():
    ...
    try:
        ...
        raise ValueError(
            "Usage: python one_st_audio_production_explicit.py [pc_type] "
...
if expInfo is None:
    ...
    dlg = gui.DlgFromDict(expInfo, title='Audio Production (explicit) - strict TSV')
    if not dlg.OK:
        psychopy.core.quit()
...
if xf.tell() == 0:
    ...
    xf.write(f"#e Task: st audio PRODUCTION (explicit)\n")
...
try:
    ...
    for i, tr in enumerate(conditions, start=1):
        ...
        # Explicit variant: subdivide long beat intervals; base unit = isi_1 (T).
        ...
        for k, ioims in enumerate(isis, start=1):
            ...
            # -------- isi_k (explicit: subdivide long beat intervals) --------
            play_isi_explicit(
                k, ioims, t_beep, audio_dur, base_unit, is_beat,
                _play_soft_beep,
                lambda *row: w(xw, [session_id, block_no, tnum,
                                    trial_label] + list(row)),
                wait_ms, ms_now)
