...
from utilities import production_rating_py, is_beat_trial, SUBINTERVAL_TAG, load_training_inputs, load_inputs
...
def load_production(subject_id, session_id, run_number, session_type='training'):
...
def _cli_exp_info():
    ...
    try:
        ...
        raise ValueError(
            "Usage: python one_st_visual_production_explicit.py [pc_type] "
...
if expInfo is None:
    ...
    dlg = gui.DlgFromDict(expInfo, title='Visual Production (explicit) - strict TSV')
    if not dlg.OK:
        psychopy.core.quit()
hide_cursor()
...
if xf.tell() == 0:
    ...
    xf.write(f"#e Task: st visual PRODUCTION (explicit)\n")
